...
decSound : {s : List A} → (l r : RegEx A) → Match (l ∣' r) s → Match (l ∣ r) s
decSound ∅ _ x = dec2 x
decSound ε       ∅ x = dec1 x
decSound ⟦ _ ⟧   ∅ x = dec1 x
...
decComp :{s : List A} → (l r : RegEx A) → Match (l ∣ r) s → Match (l ∣' r) s
decComp _ ∅ (dec2 ())
decComp ∅ _ (dec1 ())
decComp ∅ _ (dec2 x) = x
...
emptySplit : (s : List A) → Split s s []
emptySplit   []    = emp
emptySplit (_ ∷ s) = add (emptySplit s)

conSound : {s : List A} → (l r : RegEx A) → Match (l ⨁' r) s → Match (l ⨁ r) s
conSound ∅       _ ()
conSound ε       ∅ ()
conSound ⟦ _ ⟧   ∅ ()
conSound (_ ∣ _) ∅ ()
...
conεr : {r : RegEx A} → {s : List A} →  Match (ε ⨁ r) s → Match r s
conεr (con emp _ x) = x
conεr (con (add _) () _)
...
conrε' : {s : List A} → {r : RegEx A} →  Match (r ⨁ ε) s → Match r s
conrε' {[]}       (con (emp {.[]}) x _) = x
...
conComp : {s : List A} → (l r : RegEx A) → Match (l ⨁ r) s → Match (l ⨁' r) s
conComp _ ∅       (con _ _ ())
conComp ∅ ε       (con _ () _)
conComp ∅ ⟦ _ ⟧   (con _ () _)
conComp ∅ (_ ∣ _) (con _ () _)
...
starSound : {s : List A} → (r : RegEx A) → Match (r *') s → Match (r *) s
...
starComp : {s : List A} → (r : RegEx A) → Match (r *) s → Match (r *') s
